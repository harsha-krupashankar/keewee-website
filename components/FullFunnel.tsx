...
export default function FullFunnel() {
  ...
  const [progress, setProgress] = useState<number[]>(() => funnel.map(() => 0));
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      const raf = requestAnimationFrame(() => setProgress(funnel.map(() => 1)));
  ...
  return (
    <section id="full-funnel" className="py-14 md:py-16">
      <Container>
        ...
        <Reveal delay={60}>
          <h2 className="mb-2 max-w-[920px] font-display text-[32px] font-extrabold leading-[1.02] tracking-[-0.03em] text-ink sm:text-[42px] lg:text-[52px]">
            ...
            <span className="relative inline-block">
              <span className="absolute -bottom-0.5 left-[-3px] right-[-4px] h-[34%] rounded-sm bg-lime" />
        ...
        <Reveal delay={120}>
          <p className="mb-7 font-body text-base font-semibold text-body">
            We don&apos;t do marketing <em>for</em> you. We do it{" "}
            <strong className="font-bold text-green">with</strong> you.
          </p>
        </Reveal>
        ...
        <div className="flex flex-col">
          {funnel.map((stage, i) => {
            ...
            const barTarget = parseFloat(stage.barWidth);
            return (
              <div
                key={stage.no}
                ref={(el) => {
