...
/**
 * Social marks, keyed by the `platform` value in the Studio's picker
 * (`SOCIAL_PLATFORMS` in `sanity/schemaTypes/objects/primitives.ts`).
 *
 * Anything without a path here still renders — as the first two letters of its
 * platform — so a new network never leaves a blank square, it just waits for
 * artwork. Shared by the site footer and the `/links` profile so both draw the
 * same marks from the same list; only the surrounding chip differs.
 */
...
/**
 * Just the mark. The caller supplies the chip around it, because the footer's
 * circle and the `/links` rounded square are different shapes carrying the
 * same artwork.
 */
